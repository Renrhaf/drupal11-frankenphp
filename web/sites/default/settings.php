 ...
 * @endcode
 */
$settings['hash_salt'] = 'j_beWtVfuiREbTNuOqFT58-JDypXXvXC9Fd8OeTQn_36cncf0gC2pV_aNZPNIu3pe2Vplt4lrA';
 ...
 */
$settings['reverse_proxy'] = !empty(getenv('REVERSE_PROXY'));
 ...
 */
if (!empty(getenv('REVERSE_PROXY')) && !empty(getenv('REVERSE_PROXY_ADDRESS'))) {
    $settings['reverse_proxy_addresses'][] = getenv('REVERSE_PROXY_ADDRESS');
} else {
 ...
 */
$settings['file_private_path'] = $app_root . '/../data/files/private';
...
if (!empty(getenv('TRUSTED_HOST_PATTERN_ADDITIONAL'))) {
  $patterns = explode('|', getenv('TRUSTED_HOST_PATTERN_ADDITIONAL'));
 ...
 * Reroute email.
 */
$config['reroute_email.settings']['enable'] = 'TRUE' === getenv('REROUTE_EMAIL_ENABLE');
$config['reroute_email.settings']['address'] = getenv('REROUTE_EMAIL_ADDRESS');

/**
 * Monolog configurations.
 */
$settings['container_yamls'][] = $app_root . '/' . $site_path . '/default/monolog.services.yml';

/**
 * Site environment variable.
 */
$settings['SITE_ENVIRONMENT'] = getenv('APP_ENV');

/**
 * On CLI commands, increase the memory limit.
 */
if (PHP_SAPI === 'cli') {
